...
const Merger = {
   ...
   */
  async mergeAndStreamDownload(segments, segmentResults, encryption, fileName, onProgress = null) {
    const totalSize = this.estimateTotalSize(segments, Object.values(segmentResults).filter(Boolean));
    const strategy = this.decideStrategy(totalSize, this.DEFAULT_LARGE_FILE_THRESHOLD);
  ...
  async _batchMerge(segments, segmentResults, encryption, onProgress) {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (encryption && encryption.method === 'AES-128') {
        data = await this._decryptOne(segments[i], data, encryption);
      }
   ...
   * 解密单个分片
   */
  async _decryptOne(segment, data, encryption) {
    let cachedKey = null;
    if (encryption.uri) {
      const keyData = await CryptoUtils.fetchKey(encryption.uri);
      cachedKey = await CryptoUtils.importKey(keyData);
    ...
    if (cachedKey) {
      ...
      return await CryptoUtils.decryptSegment(data, cachedKey, iv);
    }
    return data;
  },
  ...
  async merge(task, settings = {}, onProgress = null) {
    ...
    if (strategy === 'memory') {
      ...
      await this.mergeAndStreamDownload(task.segments, segmentResults, task.encryption, task.name, onProgress);
    }
  },
